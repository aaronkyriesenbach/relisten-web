        ...
        <div key={source.id}>
          ...
          <Row href={`/${artistSlug}/${year}/${month}/${day}?source=${source.id}`} active={(source.id === activeSourceId) || (!activeSourceId && idx === 0)}>
            <div>
              ...
              {exists(source.avg_rating) && Number.parseFloat(source.avg_rating) > 0 && <div className="details"><div className="label">{artistSlug === 'phish' ? 'Dot Net' : 'Rating'}:</div> <div>{Number(source.avg_rating).toFixed(2)} / {source.num_ratings || source.num_reviews} {pluralize('rating', source.num_ratings || source.num_reviews)}</div></div>}
